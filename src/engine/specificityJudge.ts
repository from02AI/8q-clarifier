...
interface SpecificityRequest {
  suggestions: Pick<Suggestion, 'text' | 'why'>[];
}

interface SpecificityResponse {
  specificity: boolean[];
  reasoning: string[];
}

export async function judgeSpecificity(suggestions: Pick<Suggestion, 'text' | 'why'>[], questionNumber?: number): Promise<boolean[]> {
  ...
  const input = suggestions.map((s, i) => 
    `${i + 1}. "${s.text}" - ${s.why}`
  ).join('\n');
  ...
  try {
    ...
    if (!content) return specificityFallback(suggestions, questionNumber);
...
async function judgeAlternativesSpecificity(suggestions: Pick<Suggestion, 'text' | 'why'>[]): Promise<boolean[]> {
...
async function judgeEdgeSpecificity(suggestions: Pick<Suggestion, 'text' | 'why'>[]): Promise<boolean[]> {
...
async function judgeRiskSpecificity(suggestions: Pick<Suggestion, 'text' | 'why'>[]): Promise<boolean[]> {
...
function specificityFallback(suggestions: Pick<Suggestion, 'text' | 'why'>[], questionNumber?: number): boolean[] {
