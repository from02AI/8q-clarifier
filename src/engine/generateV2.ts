...
import { ConversationState, QuestionOutput, ScoredSuggestion, Suggestion } from '../types';
...
export async function generateQuestionV2(
  ...
  let rawPayload = sanitize(tc ? JSON.parse(tc.function.arguments) : {}, 5);
  validate(rawPayload);
  ...
  if (rawPayload.options?.length < 5) {
    ...
    if (retryTc) {
      const retryPayload = sanitize(JSON.parse(retryTc.function.arguments), 5);
  ...
  if (finalPassers.length < 3) {
    ...
    for (let attempt = 1; attempt <= 3 && !fillerSuccess; attempt++) {
      ...
      const fillerResult = await lastMileFiller(
        messages,
        finalOptions,
        finalPassers,
        missingCount,
...
async function scoreAllCandidates(
  ...
  const [relevanceScores_arr, specificityFlags, existingTexts] = await Promise.all([
    ...
    judgeSpecificity(candidates, questionNumber), // Pass question number
  ...
  return candidates.map((candidate, i): ScoredSuggestion => {
    ...
    const similarToExisting = existingTexts.some(existing => 
...
async function scoreFinalOptions(ideaCtx: string, options: Suggestion[], qNum?: number, relativeThreshold?: number) {
  ...
  const spec = await judgeSpecificity(options, qNum);
...
REQUIREMENTS:
1. Relevance: MUST achieve ≥ ${CFG.RELEVANCE_THRESH} relevance to the idea context "${ideaCtx}"
