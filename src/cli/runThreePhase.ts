...
import { runEvaluationBatch } from './runBatch';
import { SessionManager } from '../state/sessionManager';

/**
 * Three-phase evaluation plan implementation
 * Phase 0: Calibration (1 session)
 * Phase 1: Smoke test (10 sessions) 
 * Phase 2: Sample test (25 sessions)
 * Phase 3: Full evaluation (50 sessions)
 */
...
export async function runThreePhaseEvaluation(baseIdea: string = 'AI copilot for remote creative teams'): Promise<PhaseResult[]> {
  ...
  const phase0 = await runEvaluationBatch({
    sessionCount: 1,
    baseIdea,
  ...
  const phase0Result: PhaseResult = {
  ...
  const phase1 = await runEvaluationBatch({
    sessionCount: 10,
    baseIdea,
  ...
  const phase2 = await runEvaluationBatch({
    sessionCount: 25,
    baseIdea,
  ...
  const phase3 = await runEvaluationBatch({
    sessionCount: 50,
    baseIdea,
  ...
  const phase3Result: PhaseResult = {
...
export function printFinalReport(results: PhaseResult[]): void {
  ...
  console.log(`Estimated cost: $${estimatedCost.toFixed(2)} (assuming 60% output tokens at ${EFFECTIVE_CHAT_MODEL} rates)`);
