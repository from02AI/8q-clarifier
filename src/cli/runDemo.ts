...
async function main() {
  let state: ConversationState = { idea: 'AI copilot for remote creative teams', answers: [], features: {} };
  let totalMetrics = {
  ...
  for (let i=0;i<QUESTIONS.length;i++) {
    ...
    const qNum = i+1; const qText = QUESTIONS[i];
    ...
    if (metrics.finalSuccess) totalMetrics.questionsSuccessful++;
    ...
    saveJSON(`./cache/demo/q${qNum}`, 'payload', payload);
    saveJSON(`./cache/demo/q${qNum}`, 'score', score);
    saveJSON(`./cache/demo/q${qNum}`, 'metrics', metrics);
  ...
  saveJSON('./cache/demo', 'summary', totalMetrics);
}
